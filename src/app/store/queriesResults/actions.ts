...
import { DELETE_QUERY_RESULT_ACTION, LOAD_RESULTS_ACTION, SAVE_NEW_RESULT_ACTION } from "."
import { localStorage, pushSearchResultsIfNotExists } from "../../services"

const getSavedQueries = async () => JSON.parse(await localStorage.get('queries', JSON.stringify([])));
export const saveNewQueryResultAction = ({ results, buildedQuery }: any) => async(dispatch:any) =>{
...
export const deleteQueryResultAction = ({name}:any) => async(dispatch:any) =>{
    let queries = await getSavedQueries()
